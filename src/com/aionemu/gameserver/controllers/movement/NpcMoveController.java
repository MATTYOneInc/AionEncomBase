...
public class NpcMoveController extends CreatureMoveController<Npc> {
	...
	public void moveToDestination() {
		if (((Npc) this.owner).getAi2().isLogging()) {
			AI2Logger.moveinfo((Creature) this.owner,
					"\u79fb\u81f3\u76ee\u7684\u5730: " + (Object) ((Object) this.destination));
		}
	...
	protected void moveToLocation(float targetX, float targetY, float targetZ, float offset) {
		...
		if (((Npc) this.owner).getAi2().isLogging()) {
			AI2Logger.moveinfo((Creature) this.owner, "OLD targetDestX: " + this.targetDestX + " targetDestY: "
					+ this.targetDestY + " targetDestZ " + this.targetDestZ);
		}
		...
		if (((Npc) this.owner).getAi2().isLogging()) {
			...
			AI2Logger.moveinfo((Creature) this.owner, "targetDestX: " + this.targetDestX + " targetDestY: "
					+ this.targetDestY + " targetDestZ " + this.targetDestZ);
		}
		...
		if (GeoDataConfig.GEO_NPC_MOVE && GeoDataConfig.GEO_ENABLE
				&& ((Npc) this.owner).getAi2().getSubState() != AISubState.WALK_PATH
				&& ((Npc) this.owner).getAi2().getState() != AIState.RETURNING
				&& ((Npc) this.owner).getGameStats().getLastGeoZUpdate() < System.currentTimeMillis()) {
			if (((Npc) this.owner).getSpawn().getX() != this.targetDestX
					|| ((Npc) this.owner).getSpawn().getY() != this.targetDestY
					|| ((Npc) this.owner).getSpawn().getZ() != this.targetDestZ) {
				float geoZ = GeoService.getInstance().getZ(((Npc) this.owner).getWorldId(), newX, newY, newZ, 0.0f,
						((Npc) this.owner).getInstanceId());
				if (Math.abs(newZ - geoZ) > 1.0f) {
					directionChanged = true;
				}
				newZ = geoZ + ((Npc) this.owner).getObjectTemplate().getBoundRadius().getUpper()
						- ((Npc) this.owner).getObjectTemplate().getHeight();
			}
			((Npc) this.owner).getGameStats().setLastGeoZUpdate(System.currentTimeMillis() + 1000L);
		}
		if (((Npc) this.owner).getAi2().isLogging()) {
			AI2Logger.moveinfo((Creature) this.owner,
					"newX=" + newX + " newY=" + newY + " newZ=" + newZ + " mask=" + this.movementMask);
		}
	...
	public void setCurrentRoute(List<RouteStep> currentRoute) {
		if (currentRoute == null) {
			AI2Logger.info(((Npc) this.owner).getAi2(), String.format(
					"MC: \u7f6e\u5f53\u524d\u8def\u7ebf\u5c06\u8def\u7ebf\u8bbe\u7f6e\u4e3a\u7a7a (NPC id: {})!!!",
					((Npc) this.owner).getNpcId()));
		} else {
	...
	public void setRouteStep(RouteStep paramRouteStep1, RouteStep paramRouteStep2) {
		...
		if (((Npc) this.owner).getWalkerGroup() != null) {
			...
			localPoint2D = WalkerGroup.getLinePoint(new Point2D(paramRouteStep2.getX(), paramRouteStep2.getY()),
					new Point2D(paramRouteStep1.getX(), paramRouteStep1.getY()),
					((Npc) this.owner).getWalkerGroupShift());
			this.pointZ = paramRouteStep2.getZ();
			if (GeoDataConfig.GEO_ENABLE && GeoDataConfig.GEO_NPC_MOVE) {
				Vector3f localVector3f = GeoService.getInstance().getClosestCollision((Creature) this.owner,
						paramRouteStep2.getX(), paramRouteStep2.getY(), paramRouteStep2.getZ(), false, (byte) 0);
			...
			if (GeoDataConfig.GEO_ENABLE && GeoDataConfig.GEO_NPC_MOVE) {
				Vector3f localVector3f = GeoService.getInstance().getClosestCollision((Creature) this.owner,
						paramRouteStep1.getX(), paramRouteStep1.getY(), paramRouteStep1.getZ(), false, (byte) 0);
				this.pointZ = localVector3f.getZ();
	...
	public boolean isReachedPoint() {
		return MathUtil.getDistance(((Npc) this.owner).getX(), ((Npc) this.owner).getY(), ((Npc) this.owner).getZ(),
				this.pointX, this.pointY, this.pointZ) < (double) 0.05f;
	}
	...
	public void chooseNextStep() {
		...
		if (this.currentRoute == null) {
			...
			if (!WalkerFormator.processClusteredNpc((Npc) this.owner, ((Npc) this.owner).getWorldId(),
					((Npc) this.owner).getInstanceId())
					&& (template = DataManager.WALKER_DATA
							.getWalkerTemplate(((Npc) this.owner).getSpawn().getWalkerId())) != null) {
	...
	public void storeStep() {
		...
		Point3D currentStep = new Point3D(((Npc) this.owner).getX(), ((Npc) this.owner).getY(),
				((Npc) this.owner).getZ());
		if (((Npc) this.owner).getAi2().isLogging()) {
			AI2Logger.moveinfo((Creature) this.owner, "store back step: X=" + ((Npc) this.owner).getX() + " Y="
					+ ((Npc) this.owner).getY() + " Z=" + ((Npc) this.owner).getZ());
		}
		if (this.stepSequenceNr == 0
				|| MathUtil.getDistance(this.lastSteps.get(this.stepSequenceNr), currentStep) >= 5.0) {
	...
	public Point3D recallPreviousStep() {
		...
		if (result == null) {
			...
			if (((Npc) this.owner).getAi2().isLogging()) {
				AI2Logger.moveinfo((Creature) this.owner,
						"recall back step: X=" + result.getX() + " Y=" + result.getY() + " Z=" + result.getZ());
			}
