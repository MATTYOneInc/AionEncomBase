			...
			case AREA:
				...
				for (VisibleObject nextCreature : firstTarget.getKnownList().getKnownObjects().values()) {
					...
					if (properties.getDirection() == AreaDirections.FRONT) {
						...
						FloatRange range = new FloatRange(angle, -angle);
						...
						if (properties.getDirection() != AreaDirections.BACK) {
							if (!range.containsFloat(angleToTarget))
								continue;
						} else {
						...
						// Lightning bolt
						if (MathUtil.isInRange(skill.getEffector(), nextCreature, distanceToTarget) || MathUtil.isInsideAttackCylinder(skill.getEffector(), nextCreature, properties.getEffectiveDist(), properties.getEffectiveRange(),
				...
				if (skill.getEffector() instanceof Player) {
					...
					if (effector.isInAlliance2()) {
						...
						for (Player member : effector.getPlayerGroup2().getMembers()) {
							...
							if (member != null && MathUtil.isIn3dRange(effector, member, distanceToTarget + 1)) {
