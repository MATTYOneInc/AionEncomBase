 ...
 * entire table in a way that prevents all access. This class is fully inter-operable with {@code Hashtable} in programs that rely on its thread
 ...
 * entries. Similarly, Iterators and Enumerations return elements reflecting the state of the hash table at some point at or since the creation of the
 * iterator/enumeration. They do <em>not</em> throw {@link ConcurrentModificationException}. However, iterators are designed to be used by only one
 * thread at a time. Bear in mind that the results of aggregate status methods including {@code size}, {@code
 * isEmpty}, and {@code containsValue} are typically useful only when a map is not undergoing concurrent updates in other threads. Otherwise the
 * results of these methods reflect transient states that may be adequate for monitoring or estimation purposes, but not for program control.
 * <p/>
 * <p/>
 ...
 * <p/>
 * <p/>
 * A {@link Set} projection of a ConcurrentHashMapV8 may be created (using {@link #newKeySet()} or {@link #newKeySet(int)}), or viewed (using
 ...
 * <p/>
 * <p/>
 * A ConcurrentHashMapV8 can be used as scalable frequency map (a form of histogram or multiset) by using {@link LongAdder} values and initializing
 * via {@link #computeIfAbsent}. For example, to add a count to a {@code ConcurrentHashMapV8<String,LongAdder> freqs}, you can use
 ...
 * <p/>
 * <p/>
 * ConcurrentHashMapV8s support sequential and parallel operations bulk operations. (Parallel forms use the {@link ForkJoinPool#commonPool()}). Tasks
 * that may be used in other contexts are available in class {@link ForkJoinTasks}. These operations are designed to be safely, and often sensibly,
 * applied even with maps that are being concurrently updated by other threads; for example, when computing a snapshot summary of the values in a
 * shared registry. There are three kinds of operation, each with four forms, accepting functions with Keys, Values, Entries, and (Key, Value)
 * arguments and/or return values. Because the elements of a ConcurrentHashMapV8 are not ordered in any particular way, and may be processed in
 * different orders in different parallel executions, the correctness of supplied functions should not depend on any ordering, or on any other objects
 * or values that may transiently change while computation is in progress; and except for forEach actions, should ideally be side-effect-free.
 * <p/>
 * <ul>
 ...
 * <p/>
 * <p/>
 * The concurrency properties of bulk operations follow from those of ConcurrentHashMapV8: Any non-null result returned from {@code get(key)} and
 ...
 * <p/>
 * <p/>
 * <em>jsr166e note: During transition, this class uses nested functional interfaces with different names but the same forms as those expected for
 * JDK8.</em>
 * <p/>
 * <p/>
...
public class ConcurrentHashMapV8<K, V> implements ConcurrentMap<K, V>, Serializable {
	 ...
	 * <p/>
	 * <p/>
	 * Sample usage: Here is one (of the several) ways to compute the sum of the values held in a map using the ForkJoin framework. As illustrated
	 * here, Spliterators are well suited to designs in which a task repeatedly splits off half its work into forked subtasks until small enough to
	 * process directly, and then joins these subtasks. Variants of this style can also be used in completion-based designs.
	 * <p/>
	 ...
	 * int split = (n < p)? n : p;
	 * long sum = aForkJoinPool.invoke(new SumValues(m.valueSpliterator(), split, null)); // ... static class SumValues extends RecursiveTask<Long> {
	 * final Spliterator<Long> s; final int split; // split while > 1 final SumValues nextJoin; // records forked subtasks to join
	 * SumValues(Spliterator<Long> s, int depth, SumValues nextJoin) { this.s = s; this.depth = depth; this.nextJoin = nextJoin; } public Long
	 * compute() { long sum = 0; SumValues subtasks = null; // fork subtasks for (int s = split >>> 1; s > 0; s >>>= 1) (subtasks = new
	 * SumValues(s.split(), s, subtasks)).fork(); while (s.hasNext()) // directly process remaining elements sum += s.next(); for (SumValues t =
	...
	public static interface Spliterator<T> extends Iterator<T> {
		 ...
		 * Spliterator. After invoking this method, the current Spliterator will <em>not</em> produce any of the elements of the returned Spliterator,
		 * but the two Spliterators together will produce all of the elements that would have been produced by this Spliterator had this method not
		 * been called. The exact number of elements produced by the returned Spliterator is not guaranteed, and may be zero (i.e., with
	 ...
	 *
	 * Each key-value mapping is held in a Node. Because Node key fields can contain special values, they are defined using plain Object types (not
	 * type "K"). This leads to a lot of explicit casting (and many explicit warning suppressions to tell compilers not to complain about it). It also
	 * allows some of the public methods to be factored into a smaller number of internal methods (although sadly not so for the five variants of
	 * put-related operations). The validation-based approach explained below leads to a lot of code sprawl because retry-control precludes factoring
	 ...
	 * arrange this without adding further indirections, we use intrinsics (sun.misc.Unsafe) operations. The lists of nodes within bins are always
	 * accurately traversable under volatile reads, so long as lookups check hash code and non-nullness of value before checking key equality.
	 *
	 ...
	 * negative hash fields are forwarding nodes to either TreeBins or resized tables. The lower 31 bits of each normal Node's hash field contain a
	 ...
	 * remains first until deleted or the bin becomes invalidated (upon resizing). However, operations that only conditionally update may inspect
	 * nodes until the point of update. This is a converse of sorts to the lazy locking technique described by Herlihy & Shavit.
	 *
	 ...
	 * (1<<30), so some keys MUST collide. Similarly for dumb or hostile usages in which multiple keys are designed to have identical hash codes.
	 * Also, although we guard against the worst effects of this (see method spread), sets of hashes may differ only in bits that do not impact their
	 * bin index for a given power-of-two mask. So we use a secondary strategy that applies when the number of nodes in a bin exceeds a threshold, and
	 * at least one of the keys implements Comparable. These TreeBins use a balanced tree to hold nodes (a specialized form of red-black trees),
	 * bounding search time to O(log N). Each search step in a TreeBin is around twice as slow as in a regular list, but given that N cannot exceed
	 * (1<<64) (before running out of addresses) this bounds search steps, lock hold times, etc, to reasonable constants (roughly 100 nodes inspected
	 * per operation worst case) so long as keys are Comparable (which is very common -- String, Long, etc). TreeBin nodes (TreeNodes) also maintain
	 * the same "next" traversal pointers as regular nodes, so can be traversed in iterators in the same way.
	 *
	 ...
	 * Resizing proceeds by transferring bins, one by one, from the table to the next table. To enable concurrency, the next table must be
	 * (incrementally) prefilled with place-holders serving as reverse forwarders to the old table. Because we are using power-of-two expansion, the
	 * elements from each bin must either stay at same index, or move with a power of two offset. We eliminate unnecessary node creation by catching
	 * cases where old nodes can be reused because their next fields won't change. On average, only about one-sixth of them need cloning when a table
	 * doubles. The nodes they replace will be garbage collectable as soon as they are no longer referenced by any reader thread that may be in the
	 * midst of concurrently traversing table. Upon transfer, the old table bin contains only a special forwarding node (with hash field "MOVED") that
	 * contains the next table as its key. On encountering a forwarding node, access and update operations restart, using the new table.
	 *
	 ...
	 * that all accessible bins in both the old and new table are usable by any traversal. This is arranged by proceeding from the last bin
	 ...
	 * without revisiting nodes. However, to ensure that no intervening nodes are skipped, bin splitting can only begin after the associated
	 * reverse-forwarders are in place.
	 *
	 ...
	 * at threshold is around 13%, meaning that only about 1 in 8 puts check threshold (and after resizing, many fewer do so). The bulk putAll
	 * operation further reduces contention by only committing count updates upon these size checks.
	 *
	...
	/**
	 * The bin count threshold for using a tree rather than list for a bin. The value reflects the approximate break-even point for using tree-based
	 * operations.
	 */
	...
	/**
	 * Holder for the thread-local hash code determining which CounterCell to use. The code is initialized via the counterHashCodeGenerator, but may
	 * be moved upon collisions.
	 */
	...
	/*
	 * Volatile access methods are used for table elements as well as elements of in-progress next table while resizing. Uses are null checked by
	 * callers, and implicitly bounds-checked, relying on the invariants that tab arrays have non-zero size, and all indices are masked with
	 * (tab.length - 1) which is never negative and always less than length. Note that, to be correct wrt arbitrary concurrency errors by users,
	 * bounds checks must operate on local variables, which accounts for some odd-looking inline assignments below.
	 */
	...
	/**
	 * Key-value entry. Note that this is never exported out as a user-visible Map.Entry (see MapEntry below). Nodes with a hash field of MOVED are
	 * special, and do not contain user keys or values. Otherwise, keys are never null, and null val fields indicate that a node is in the process of
	 * being deleted or created. For purposes of read-only access, a key may be read before a val, but can only be used after checking val to be
	 * non-null.
	 */
	static class Node<V> {
	 ...
	 * Comparable<T> for the same T, so we cannot invoke compareTo among them. To handle this, the tree is ordered primarily by hash value, then by
	 * getClass().getName() order, and then by Comparator order among elements of the same class. On lookup at a node, if elements are not comparable
	 * or compare as 0, both left and right children may need to be searched in the case of tied hash values. (This corresponds to the full list
	 * search that would be necessary if all elements were non-Comparable and had tied hashes.) The red-black balancing code is updated from
	 * pre-jdk-collections (http://gee.cs.oswego.edu/dl/classes/collections/RBCell.java) based in turn on Cormen, Leiserson, and Rivest "Introduction
	 * to Algorithms" (CLR).
	 * <p/>
	 * TreeBins also maintain a separate locking discipline than regular bins. Because they are forwarded via special MOVED nodes at bin heads (which
	 * can never change once established), we cannot use those nodes as locks. Instead, TreeBin extends AbstractQueuedSynchronizer to support a simple
	 * form of read-write lock. For update operations and table validation, the exclusive form of lock behaves in the same way as bin-head locks.
	 * However, lookups use shared read-lock mechanics to allow multiple readers in the absence of writers. Additionally, these lookups do not ever
	 * block: While the lock is not available, they proceed along the slow traversal path (via next-pointers) until the lock becomes available or the
	 * list is exhausted, whichever comes first. (These cases are not fast, but maximize aggregate expected throughput.) The AQS mechanics for doing
	 * this are straightforward. The lock state is held as AQS getState(). Read counts are negative; the write count (1) is positive. There are no
	 * signalling preferences among readers and writers. Since we don't need to export full Lock API, we just override the minimal AQS methods and use
	 * them directly.
	 */
	static final class TreeBin<V> extends AbstractQueuedSynchronizer {
		...
		/**
		 * Wrapper for getTreeNode used by CHM.get. Tries to obtain read-lock to call getTreeNode, but during failure to get lock, searches along next
		 * links.
		 */
	...
	/**
	 * Spreads higher bits to lower, and also forces top bit to 0. Because the table uses power-of-two masking, sets of hashes that vary only in bits
	 * above the current mask will always collide. (Among known examples are sets of Float keys holding consecutive whole numbers in small tables.) To
	 * counter this, we apply a transform that spreads the impact of higher bits downward. There is a tradeoff between speed, utility, and quality of
	 * bit-spreading. Because many common sets of hashes are already reasonably distributed across bits (so don't benefit from spreading), and because
	 * we use trees to handle large sets of collisions in bins, we don't need excessively high quality.
	 */
	...
	/*
	 * Internal versions of insertion methods All have the same basic structure as the first (internalPut): 1. If table uninitialized, create 2. If
	 * bin empty, try to CAS new node 3. If bin stale, use new table 4. if bin converted to TreeBin, validate and relay to TreeBin methods 5. Lock and
	 * validate; if valid, scan and add or update
	 *
	 * The putAll method differs mainly in attempting to pre-allocate enough table space, and also more lazily performs count updates and checks.
	 *
	 * Most of the function-accepting methods can't be factored nicely because they require different functional forms, so instead sprawl out similar
	 * mechanics.
	 */
	...
	private final void addCount(long x, int check) {
		...
		if ((as = counterCells) != null || !U.compareAndSwapLong(this, BASECOUNT, b = baseCount, s = b + x)) {
			...
			if ((hc = threadCounterHashCode.get()) == null || as == null || (m = as.length - 1) < 0 || (a = as[m & hc.code]) == null
	 ...
	 * <p/>
	 * At each step, the iterator snapshots the key ("nextKey") and value ("nextVal") of a valid node (i.e., one that, at point of snapshot, has a
	 * non-null user value). Because val fields can change (including to null, indicating deletion), field nextVal might not be accurate at point of
	 * use, but still maintains the weak consistency property of holding a value that was once valid. To support iterator.remove, the nextKey field is
	 * not updated (nulled out) when the iterator cannot advance.
	 * <p/>
	 * Internal traversals directly access these fields, as in: {@code while (it.advance() != null) { process(it.nextKey); }}
	 * <p/>
	 * Exported iterators must track whether the iterator has advanced (in hasNext vs next) (by setting/checking/nulling field nextVal), and then
	 * extract key, value, or key-value pairs as return values of next().
	 * <p/>
	 * The iterator visits once each still-valid node that was reachable upon iterator construction. It might miss some that were added to a bin after
	 * the bin was visited, which is OK wrt consistency guarantees. Maintaining this property in the face of possible ongoing resizes requires a fair
	 * amount of bookkeeping state that is difficult to optimize away amidst volatile accesses. Even so, traversal maintains reasonable throughput.
	 * <p/>
	 ...
	 * <p/>
	 * This class extends CountedCompleter to streamline parallel iteration in bulk operations. This adds only a few fields of space overhead, which
	 * is small enough in cases where it is not needed to not worry about it. Because CountedCompleter is Serializable, but iterators need not be, we
	...
	static class Traverser<K, V, R> extends CountedCompleter<R> {
		...
		/**
		 * Returns a batch value > 0 if this task should (and must) be split, if so, adding to pending count, and in any case updating batch value.
		 * The initial batch value is approx exp2 of the number of times (minus one) to split task by two before executing leaf action. This value is
		 * faster to compute and more convenient to use as a guide to splitting than is the depth, since it is used while dividing by two anyway.
		 */
		final int preSplit() {
			ConcurrentHashMapV8<K, V> m;
			int b;
	...
	/**
	 * Returns the number of mappings. This method should be used instead of {@link #size} because a ConcurrentHashMapV8 may contain more mappings
	 * than can be represented as an int. The value returned is an estimate; the actual count may differ if there are concurrent insertions or
	 * removals.
	 *
	 ...
	 * {@link #containsValue}, and exists solely to ensure full compatibility with class {@link java.util.Hashtable}, which supported this method
	 ...
	 * unless null. This is equivalent to
	 * 
	 * <pre>
	 *  {@code
	 ...
	 * </pre>
	 * <p/>
	 * except that the action is performed atomically. If the function returns {@code null} no mapping is recorded. If the function itself throws an
	 * (unchecked) exception, the exception is rethrown to its caller, and no mapping is recorded. Some attempted update operations on this map by
	 * other threads may be blocked while computation is in progress, so the computation should be short and simple, and must not attempt to update
	 * any other mappings of this Map. The most appropriate usage is to construct a new object serving as an initial mapped value, or memoized result,
	 * as in:
	 * <p/>
	 ...
	 * </pre>
	 * <p/>
	 * except that the action is performed atomically. If the function returns {@code null}, the mapping is removed. If the function itself throws an
	 * (unchecked) exception, the exception is rethrown to its caller, and the current mapping is left unchanged. Some attempted update operations on
	 * this map by other threads may be blocked while computation is in progress, so the computation should be short and simple, and must not attempt
	 * to update any other mappings of this Map. For example, to either create or append new messages to a value mapping:
	 *
	 * @param key key with which the specified value is to be associated
	 ...
	 * < pre> {@code
	 * Map<Key, String> map = ...;
	 * final String msg = ...; map.compute(key, new BiFun<Key, String, String>() { public String apply(Key k, String v) { return (v == null) ? msg : v
	 * + msg;});}}
	 * </pre>
	 *
	 * @param key key with which the specified value is to be associated
	...
	/**
	 * If the specified key is not already associated with a value, associate it with the given value. Otherwise, replace the value with the results
	 * of the given remapping function. This is equivalent to:
	 * 
	 * <pre>
	 *  {@code
	 *   if (!map.containsKey(key))
	 ...
	 * {@link Collection#addAll}). This is of course only appropriate if it is acceptable to use the same value for all additions from this view.
	 *
	 ...
	 * <p/>
	 * <p/>
	 * The view's {@code iterator} is a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to
	 ...
	 *
	 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case the action is
	 * not applied)
	 * @param action the action
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * none.
	 *
	 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case it is not
	 * combined)
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of keys, in which additions may optionally be enabled by mapping to a common value. This class
	 * cannot be directly instantiated. See null null null null null null {@link #keySet()}, {@link #keySet(Object)}, {@link #newKeySet()},
	 * {@link #newKeySet(int)}.
	 */
	public static class KeySetView<K, V> extends CHMView<K, V> implements Set<K>, java.io.Serializable {
		...
		/**
		 * Returns a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to traverse elements
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Collection} of values, in which additions are disabled. This class cannot be directly instantiated.
	 * See {@link #values()}.
	 * <p/>
	 * <p/>
	 * The view's {@code iterator} is a "weakly consistent" iterator that will never throw {@link ConcurrentModificationException}, and guarantees to
	 ...
	 * operations. Each method has the same name, but returns a task rather than invoking it. These methods may be useful in custom applications such
	 * as submitting a task without waiting for completion, using a custom pool, or combining with other tasks.
	 */
	public static class ForkJoinTasks {
		 ...
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case the action
		 * is not applied)
		 * @param action the action
		 * @return the task
		 */
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all (key, value) pairs using the given
		...
		/**
		 * Returns a task that when invoked, returns a non-null result from applying the given search function on each key, or null if none. Upon
		 * success, further element processing is suppressed and the results of any other parallel invocations of the search function are ignored.
		 *
		 * @param map the map
		 * @param searchFunction a function returning a non-null result on success, else null
		 * @return the task
		...
		/**
		 * Returns a task that when invoked, returns a non-null result from applying the given search function on each value, or null if none. Upon
		 * success, further element processing is suppressed and the results of any other parallel invocations of the search function are ignored.
		 *
		 * @param map the map
		 * @param searchFunction a function returning a non-null result on success, else null
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all values using the given reducer to
		 * combine values, or null if none.
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case it is not
		...
		/**
		 * Returns a task that when invoked, returns a non-null result from applying the given search function on each entry, or null if none. Upon
		 * success, further element processing is suppressed and the results of any other parallel invocations of the search function are ignored.
		 *
		 * @param map the map
		 * @param searchFunction a function returning a non-null result on success, else null
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating all entries using the given reducer to combine values, or null if
		 * none.
		 *
		 * @param map the map
		 * @param reducer a commutative associative combining function
		...
		/**
		 * Returns a task that when invoked, returns the result of accumulating the given transformation of all entries using the given reducer to
		 * combine values, or null if none.
		 *
		 * @param map the map
		 * @param transformer a function returning the transformation for an element, or null if there is no transformation (in which case it is not
